...
public class StatistiquesController {
    ...
    public StatistiquesController(DossierRepository dossierRepository, EtapeDossierRepository etapeDossierRepository) {
    ...
    public Long getNombreTotalDossiers() {
        return (long) dossierRepository.findAll().size();
    }
    ...
    public ResponseEntity<byte[]> exportPdf() throws IOException {
        StatistiquesSynthese s = synthese();
        byte[] bytes = generatePdf(s);
    ...
    private byte[] generatePdf(StatistiquesSynthese s) throws IOException {
        ...
        try (PDDocument doc = new PDDocument()) {
            ...
            cs.beginText(); cs.setFont(fontBold, 15); cs.newLineAtOffset(margin, y[0]); cs.showText("CARFO — Synthèse des statistiques"); cs.endText();
            y[0] -= 18;
    ...
    private byte[] generateExcel(StatistiquesSynthese s) throws IOException {
        try (XSSFWorkbook wb = new XSSFWorkbook(); ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            ...
            org.apache.poi.ss.usermodel.Font hfont = wb.createFont(); hfont.setBold(true); headerStyle.setFont(hfont);
    ...
    private static int kv(Sheet sheet, int r, String label, String value, org.apache.poi.ss.usermodel.CellStyle headerStyle) {
        Row row = sheet.createRow(r);
    ...
    private byte[] generateWord(StatistiquesSynthese s) throws IOException {
        try (XWPFDocument doc = new XWPFDocument(); ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            ...
            titleRun.setText("CARFO — Synthèse des statistiques");

            org.apache.poi.xwpf.usermodel.XWPFParagraph sub = doc.createParagraph();
            org.apache.poi.xwpf.usermodel.XWPFRun subRun = sub.createRun();
            ...
            if (s.getTop5Risques() != null) {
                for (StatistiquesSynthese.DossierLeger dl : s.getTop5Risques()) {
                    org.apache.poi.xwpf.usermodel.XWPFTableRow row = top5.createRow();
                    ...
                    for (int i = 0; i < vals.length; i++) {
                        org.apache.poi.xwpf.usermodel.XWPFTableCell cell = i < row.getTableCells().size() ? row.getCell(i) : row.addNewTableCell();
                        cell.setText(vals[i]);
                    }
                }
            }
    ...
    private static void fillKvRow(org.apache.poi.xwpf.usermodel.XWPFTableRow row, String label, String value) {
        row.getCell(0).setText(label);
        org.apache.poi.xwpf.usermodel.XWPFTableCell valueCell = row.getTableCells().size() > 1 ? row.getCell(1) : row.addNewTableCell();
